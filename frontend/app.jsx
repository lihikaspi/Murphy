...
import { Shield, AlertTriangle, ArrowRight, RefreshCw, Activity, Clock, Terminal, ChevronDown, History, Zap } from 'lucide-react';
...
const Sidebar = ({ onReset, mode }) => (
  <div className="w-64 bg-white border-r border-gray-200 h-screen flex flex-col p-4 fixed left-0 top-0 z-10 hidden md:flex">
    <div className="flex items-center justify-center mb-8 mt-2">
      <div className="w-12 h-12 bg-gray-900 rounded-full flex items-center justify-center text-white font-bold text-xl mr-3 shadow-lg">
        M
      </div>
          ...
          <div className="space-y-6">
            <div className="glass-panel p-5 rounded-xl">
              ...
              <textarea
                ...
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black focus:outline-none bg-gray-50 h-32 resize-none overflow-y-auto"
                placeholder="I am a software engineer building a SaaS..."
                required
              />
            </div>
            <div className="glass-panel p-5 rounded-xl">
          ...
          <div className="space-y-6">
            <div className="glass-panel p-5 rounded-xl">
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black focus:outline-none bg-gray-50 h-32 resize-none overflow-y-auto"
                placeholder="1. Launch Product Hunt..."
                required
              />
            </div>
            <div className="glass-panel p-5 rounded-xl">
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black focus:outline-none bg-gray-50 h-32 resize-none overflow-y-auto"
                placeholder="Server might crash..."
              />
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="w-full md:w-1/2">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Pessimism Level (Murphy's Law)
            </label>
            <div className="relative">
              <select
                name="pessimism"
                value={formData.pessimism}
                onChange={handleChange}
                className="w-full p-3 bg-white border border-gray-200 rounded-lg appearance-none cursor-pointer focus:ring-2 focus:ring-black focus:outline-none font-medium text-gray-700"
              >
                {PESSIMISM_OPTIONS.map(opt => (
          ...
          <button
            ...
            className="w-full md:w-auto px-8 py-3 bg-gray-900 text-white font-bold rounded-lg shadow-lg hover:bg-gray-800 transition-all flex items-center justify-center gap-2 disabled:opacity-70"
          >
  ...
  return (
    <div className="max-w-2xl mx-auto pt-16 fade-in px-4 md:px-0">
      <div className="bg-red-50 border border-red-100 rounded-2xl p-8 mb-8 shadow-sm">
        ...
        <div className="bg-white/50 p-4 rounded-lg text-sm text-red-800 font-medium">
      ...
      <div className="glass-panel p-8 rounded-2xl">
        ...
        <div className="mt-8 flex justify-end">
          <button
            ...
            className="px-6 py-3 bg-red-600 text-white font-bold rounded-lg shadow-lg hover:bg-red-700 transition-all flex items-center gap-2"
          >
...
const DashboardView = ({ dashboardData, userPlan, onRefine, initialPessimism, feedbackHistory }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto pt-6 fade-in px-4 md:px-0 pb-12">
      ...
      {/* Main Grid Content - Standard Grid with fixed height cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">

        {/* Top Left: Original Plan */}
        <div className="glass-panel p-5 rounded-xl flex flex-col h-80 relative group overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-2 text-gray-600 text-sm whitespace-pre-wrap">
            {userPlan}
          </div>
        </div>

        {/* Top Right: Critical Failures */}
        <div className="glass-panel p-5 rounded-xl flex flex-col h-80 border-l-4 border-red-500 bg-red-50/30 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-2 text-gray-800 text-sm whitespace-pre-wrap leading-relaxed">
            {dashboardData.problems}
          </div>
        </div>

        {/* Bottom Left: Optimized Strategy */}
        <div className="glass-panel p-5 rounded-xl flex flex-col h-80 border-l-4 border-blue-500 bg-blue-50/30 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-2 text-gray-800 text-sm whitespace-pre-wrap leading-relaxed">
            {dashboardData.revised_plan}
          </div>
        </div>

        {/* Bottom Right: Improvements */}
        ...
        <div className="glass-panel w-full md:w-2/3 p-4 rounded-xl flex flex-col justify-between h-64">
          <div className="flex-1 flex flex-col">
            ...
            <textarea
              ...
              placeholder="Critique the plan or ask for specific changes (e.g., 'Make it cheaper', 'Focus on security')..."
              className="w-full bg-white border border-gray-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black resize-none overflow-y-auto flex-1"
              onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleUpdate())}
            />
          </div>
          ...
          <div className="flex items-center gap-3 mt-3">
             <div className="relative w-48">
                <select
                  value={currentPessimism}
                  onChange={(e) => setCurrentPessimism(Number(e.target.value))}
                  className="w-full pl-3 pr-8 py-2 bg-gray-50 border border-gray-200 rounded-md text-xs font-medium appearance-none cursor-pointer focus:ring-2 focus:ring-black focus:outline-none"
                >
                   {PESSIMISM_OPTIONS.map(opt => (
                     <option key={opt.value} value={opt.value}>{opt.label}</option>
                   ))}
            ...
            <button
              ...
              disabled={isUpdating}
              className="flex-1 px-4 py-2 bg-black text-white rounded-md text-sm font-bold hover:bg-gray-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
...
export default function App() {
  const [view, setView] = useState("INPUT"); // INPUT, CHOICE, DASHBOARD
  const [mode, setMode] = useState("Checking..."); // Backend or Simulation
  const [userInput, setUserInput] = useState({});
  const [currentScenario, setCurrentScenario] = useState(null);
  const [dashboardData, setDashboardData] = useState(null);
  const [feedbackHistory, setFeedbackHistory] = useState([]);

  // Mock Data Generator (Simulates backend)
  const mockGenerator = {
    scenario: (data) => ({
      ...
      <div className="md:pl-64 min-h-screen">
        ...
        {view === "DASHBOARD" && dashboardData && (
          <DashboardView
            ...
            userPlan={userInput.plan}
            initialPessimism={Number(userInput.pessimism)}
